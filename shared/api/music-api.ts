...
export function detectSongLanguage(song: Partial<Song>): string {
  if (song.language && song.language.trim()) {
    const raw = song.language.trim().toLowerCase()
    if (raw === 'ta' || raw === 'tam' || raw === 'tamil' || raw === 'தமிழ்') return 'tamil'
    if (raw === 'te' || raw === 'tel') return 'telugu'
    if (raw === 'hi' || raw === 'hin') return 'hindi'
    if (raw === 'ml' || raw === 'mal') return 'malayalam'
    if (raw === 'kn' || raw === 'kan') return 'kannada'
    if (raw === 'en' || raw === 'eng') return 'english'
    if (raw === 'pa' || raw === 'pan') return 'punjabi'
    return raw
  }
  ...
  // 3. Language-specific movie/track signatures BEFORE cross-language artists
  if (
    /\b(?:devara|pushpa|guntur kaaram|salaar|kalki|hanuman|jersey|geetha govindam|rangasthalam|arjun reddy|baahubali|rrr|ala vaikunthapurramuloo|chiranjeevi|balakrishna|pawan kalyan|mahesh babu|allu arjun|prabhas|ram charan|jr ntr|nani|vijay deverakonda|aditya music|lahari music|chuttamalle|daavudi|fear song|kurchi madathapetti|inkem inkem|samajavaragamana|butta bomma|srivalli|oo antava|naatu naatu|ramuloo|kalavathi)\b/i.test(
      combined
    )
  ) {
    return 'telugu'
  ...
  if (
    /\b(?:jawan|pathaan|dunki|animal|brahmastra|kabir singh|stree|fighter|chaleya|zinda banda|kesariya|tum hi ho|channa mereya|apna bana le|kun faya kun|chaiyya chaiyya|agar tum saath ho|shah rukh khan|salman khan|aamir khan|ranbir kapoor|ranveer singh|t-series|zee music|yrf)\b/i.test(
      combined
    )
  ) {
    return 'hindi'
  ...
  if (
    /\b(?:aavesham|illuminati|jaada|arm|manjummel boys|premalu|minnal murali|hridhyam|darshana|lucifer|drishyam|mohanlal|mammootty|fahadh faasil|dulquer|tovino|prithviraj|nivin pauly|manorama music)\b/i.test(
      combined
    )
  ) {
    return 'malayalam'
  ...
  if (
    /\b(?:kantara|kgf|charlie 777|vikrant rona|yuva|tagaru|sudeep|yash|darshan|puneeth rajkumar|shiva rajkumar|rishab shetty|anand audio)\b/i.test(
      combined
    )
  ) {
    return 'kannada'
  ...
  if (
    /\b(?:speed records|white hill music|diljit dosanjh|sidhu moose|ap dhillon|karan aujla|honey singh|guru randhawa|ammy virk|b praak|tauba tauba)\b/i.test(
      combined
    )
  ) {
    return 'punjabi'
  ...
  if (
    /\b(?:ed sheeran|taylor swift|billie eilish|the weeknd|dua lipa|coldplay|eminem|drake|post malone|bruno mars|justin bieber|alan walker|maroon 5|imagine dragons|charlie puth|selena gomez|ariana grande|faded|believer)\b/i.test(
      combined
    )
  ) {
    return 'english'
  ...
  // 4. Transliterated Tamil song/title keywords (handles English/transliterated titles)
  if (
    /\b(?:kadhal|anbe|anbenum|kanmani|nenjame|uyire|thangamey|kanave|aaruyire|sirikadhey|rowdy baby|kavithai|chellamma|morrakka|vaathi|arabic kuthu|naa ready|hukum|matta|spark|sawadeeka|katchi sera|aasa kooda|po nee po|munbe vaa|vaseegara|kurumugil|oru naalil|ennodu nee irundhal|adiga adiga|usure pogudhey|kannazhaga|kuthu|marana gana|dappankuthu|thiruvizha|paatu|paadal|paadatha|thalapathy|thalaivar|jailer|leo|goat|amaran|viruman|soorarai pottru|master|vikram|kolamaavu|beast|ponniyin selvan|varisu|viduthalai|kanguva|vettaiyan|coolie|raayan|maharaja)\b/i.test(
      combined
    )
  ) {
    return 'tamil'
  ...
  // 5. Prominent language-specific music directors / labels
  if (/thaman|devi sri prasad|dsp|keeravani/i.test(combined)) return 'telugu'
  if (/sushin shyam|dabzee|jassie gift|shaan rahman|gopi sundar|hesham/i.test(combined)) return 'malayalam'
  if (
    /arijit singh|pritam|badshah|shreya ghoshal|amitabh bhattacharya|armaan malik|neha kakkar|kumar sanu|kishore|atif aslam/i.test(
      combined
    )
  )
    return 'hindi'

  // Cross-language composers only when no other language matched
  if (
    /anirudh|a\.?r\.?\s?rahman|yuvan|harris jayaraj|vidyasagar|deva|ilayaraja|ilaiyaraaja|santhosh narayanan|g\.?v\.?\s?prakash|dhibu|sai abhyankkar|pradeep kumar|dhanush|vijay|sivaangi|jonita|sean roldan|sam c\.?s\.?|hiphop tamizha|leon james|nivas k prasanna|stephen zechariah|dharan kumar|d\.?\s?imman|ghibran|vivek-mervin|kaber vasuki/i.test(
      combined
    )
  ) {
    return 'tamil'
...
function mapToSong(item: any): Song {
  ...
  if (item.more_info && (item.more_info.encrypted_media_url || item.more_info.media_url)) {
    ...
    const thumbnailUrl =
      typeof rawImg === 'string'
        ? rawImg
            ...
            .replace('images.saavncdn.com', 'c.saavncdn.com')
        : ''
    ...
    const artistsList =
      primaryArtists.length > 0
        ? primaryArtists
        : item.subtitle
          ? item.subtitle.split(',').map((s: string) => s.trim())
          : []
  ...
  if (item.name && (item.downloadUrl || item.audioUrl)) {
    ...
    const artistsList =
      item.artists?.primary?.map((a: any) => a.name) ||
...
function ensureDistinctThumbnails(songs: Song[]): Song[] {
  ...
  return songs.map((song) => {
    if (
      song.thumbnailUrl &&
      (imageCounts.get(song.thumbnailUrl) || 0) >= 3 &&
      song.videoId &&
...
export class MusicApiClient implements MusicProvider {
  ...
  async searchSongs(query: string, maxResults = 25): Promise<Song[]> {
      ...
      const fetchSubQuery = async (q: string): Promise<Song[]> => {
        try {
          const res = await fetch(
            `https://saavn-api-seven.vercel.app/api/search/songs?query=${encodeURIComponent(q)}&limit=6`,
    ...
    if (isPureCategorySearch && (lower.includes('melody') || lower.includes('melodies'))) {
      ...
      const fetchSubQuery = async (q: string): Promise<Song[]> => {
        try {
          const res = await fetch(
            `https://saavn-api-seven.vercel.app/api/search/songs?query=${encodeURIComponent(q)}&limit=8`,
            { signal: AbortSignal.timeout(4000) }
          )
          if (res.ok) {
            const data = await res.json()
  ...
  async getTrending(preferredLanguages?: string[], maxResults = 120): Promise<Song[]> {
    ...
    const languageQueryMap: Record<string, string[]> = {
      tamil: tamilQueries,
      ta: tamilQueries,
      telugu: ['Latest Telugu Hits 2025', 'Telugu Super Hits', 'Thaman S Telugu Hits', 'Devi Sri Prasad Telugu Hits'],
      hindi: [
      ...
      malayalam: ['Latest Malayalam Hits 2025', 'Sushin Shyam Malayalam Hits', 'Malayalam Melody Hits'],
      english: ['Top Global Pop Hits 2025', 'Billboard Hot 100 Hits', 'Ed Sheeran Popular Songs'],
      kannada: ['Latest Kannada Hits 2025', 'Kannada Super Hits', 'Ravi Basrur Hits'],
      punjabi: ['Latest Punjabi Hits 2025', 'Top Punjabi Songs', 'Diljit Dosanjh Hits']
    }
    ...
    if (preferredLanguages && preferredLanguages.length > 0) {
      for (const lang of preferredLanguages) {
        ...
        const cleanLang = rawLang === 'ta' || rawLang === 'tam' ? 'tamil' : rawLang
        const queries = languageQueryMap[cleanLang]
        if (queries) {
          trendingQueries.push(...queries)
        } else {
          trendingQueries.push(`Latest ${cleanLang} Hits`)
        }
      }
    }
    ...
    const cleanTargetLangs = (preferredLanguages && preferredLanguages.length > 0 ? preferredLanguages : ['tamil']).map(
      (l) => {
        const raw = l.trim().toLowerCase()
        return raw === 'ta' || raw === 'tam' ? 'tamil' : raw
      }
    )

    try {
      const resultsArray = await Promise.all(trendingQueries.map((q) => this.searchSongs(q, 20).catch(() => [])))
      ...
      const filtered = distinct.filter((song) => {
        ...
        if (song.playCountNumber !== undefined && song.playCountNumber > 0 && song.playCountNumber < 100) {
          return false
        }

        // Strict language verification
        const songLang = song.language || detectSongLanguage(song)
        if (cleanTargetLangs.includes('tamil')) {
          if (songLang !== 'tamil') return false
        } else if (songLang && !cleanTargetLangs.includes(songLang)) {
          return false
        }
    ...
    const fallbackQuery = cleanTargetLangs.includes('tamil')
      ? 'Latest Tamil Hits'
      : `Latest ${cleanTargetLangs[0]} Hits`
    const fallback = await this.searchSongs(fallbackQuery, maxResults)
    return fallback.filter((song) => {
      const title = song.title.toLowerCase()
      if (title.includes('trending') || title.includes('jukebox') || title.includes('remaster')) return false
      const songLang = song.language || detectSongLanguage(song)
      if (cleanTargetLangs.includes('tamil')) {
        return songLang === 'tamil'
      }
      return !songLang || cleanTargetLangs.includes(songLang)
    })
  }
   ...
   */
  async getNewReleases(languages: string[] = ['tamil'], days = 30, limit = 35): Promise<Song[]> {
    const cutoffTimestamp = Date.now() - days * 86400000
    const newReleaseQueryMap: Record<string, string[]> = {
      tamil: [
        'Latest Tamil Hits 2025 2026',
        'Tamil Top Hits 2026',
        'Latest Tamil Singles',
        'Think Indie Tamil',
        'Sai Abhyankkar Hits',
        'Tamil Movie Songs 2025 2026',
        'Tamil Singles 2025 2026'
      ],
      telugu: ['Latest Telugu Hits 2025 2026', 'Telugu Super Hits'],
      hindi: ['Latest Bollywood Hindi Hits 2025 2026', 'Arijit Singh Super Hits'],
      malayalam: ['Latest Malayalam Hits 2025 2026', 'Sushin Shyam Malayalam Hits'],
      english: ['Top Global Pop Hits 2025 2026', 'Billboard Hot 100 Hits'],
      kannada: ['Latest Kannada Hits 2025 2026'],
      punjabi: ['Latest Punjabi Hits 2025 2026']
    }
    const queries = languages.flatMap((lang) => newReleaseQueryMap[lang.toLowerCase()] || [`Latest ${lang} Hits`])

    try {
      const results = await Promise.all(queries.map((q) => this.searchSongs(q, 25).catch(() => [])))
      ...
      const recent = all.filter((song) => {
        ...
        // Exclude obscure spam tracks with low plays
        if (song.playCountNumber !== undefined && song.playCountNumber > 0 && song.playCountNumber < 100) {
          return false
        }

        const isRecentYear =
          song.releaseDate?.includes('2026') ||
          song.releaseDate?.includes('2025') ||
      ...
      // Fallback: return top authentic tracks strictly matching the target languages
      const fallback = all
        .filter((song) => {
          const title = song.title.toLowerCase()
          if (title.includes('remaster') || title.includes('jukebox')) return false
          const detectedLang = detectSongLanguage(song)
          return languages.some((l) => l.toLowerCase() === detectedLang)
        })
        .sort((a, b) => (b.playCountNumber || 0) - (a.playCountNumber || 0))

      const merged = deduplicateSongs([...recent, ...fallback])
      if (merged.length > 0) {
  ...
  async getRelatedArtists(artistName: string, language?: string): Promise<string[]> {
    ...
    if (knownCollaborators[key]) {
      return await Promise.resolve(knownCollaborators[key])
    }

    const lang = language?.toLowerCase()
    if (lang === 'telugu') {
      return await Promise.resolve(['Thaman S', 'Devi Sri Prasad', 'M.M. Keeravani'])
    }
    if (lang === 'hindi') {
      return await Promise.resolve(['Arijit Singh', 'Pritam', 'Atif Aslam'])
    }
    if (lang === 'malayalam') {
      return await Promise.resolve(['Sushin Shyam', 'Dabzee', 'Shaan Rahman'])
    }

    return await Promise.resolve(['Anirudh Ravichander', 'A.R. Rahman', 'Yuvan Shankar Raja'])
  }
