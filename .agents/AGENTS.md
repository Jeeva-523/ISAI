...
- **Release Signing Keystore**: Use `android-app/debug.keystore` (alias: `androiddebugkey`) for release builds to maintain exact signature compatibility with all existing deployed user devices so users can update seamlessly without uninstalling.
